		...
		if ( $featured_image && ( ! is_post_type_archive() && ! is_archive() && ! is_home() ) ) {
			...
			if ( $media_id ) {
				...
				if ( $image ) {
					...
					$image = "<img class='l-wd-full' src='$src' alt='' srcset='$srcset' sizes='$sizes' width='$width' height='$height'>";
				}
			}

			if ( $image ) {
				$swoop = (
					'<svg class="c-hero__swoop l-absolute l-top-0 l-left-0" aria-hidden="true" focusable="false" role="img" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">' .
						'<defs>' .
