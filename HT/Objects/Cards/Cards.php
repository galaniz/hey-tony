...
class Cards {
	...
	public static function render_card( $args = [] ) {
		...
		if ( $pretitle ) {
			...
			$pretitle_output = (
				'<div class="o-overlap-v__p e-underline-r p-' . ( $first_two ? 's' : 'xs' ) . ' t-wt-bold l-pb-2xs l-flex">' .
					$pretitle .
				'</div>'
			);
		} else {
		...
		if ( $excerpt ) {
			$excerpt = (
				'<div class="p-s t-foreground-base l-relative l-zi-2">' .
